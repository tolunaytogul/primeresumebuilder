...
const CreativeWebTemplate: React.FC<CreativeWebTemplateProps> = ({ cvData }) => {
  ...
  return (
    <div className="bg-white flex flex-col xl:flex-row min-h-[600px] xl:min-h-[800px] font-sans text-sm leading-relaxed">
      {/* Sidebar */}
      <div className="w-full xl:w-2/5 bg-gradient-to-b from-purple-900 to-purple-800 text-white p-4 sm:p-6 xl:p-8">
        ...
        <div className="mb-6 sm:mb-8 text-center xl:text-left">
          <h1 className="text-xl sm:text-2xl xl:text-3xl font-bold mb-2 uppercase tracking-wide break-words">
            {personalInfo.name || 'Full Name'}
          </h1>
          <h2 className="text-base sm:text-lg xl:text-xl text-purple-200 mb-4 sm:mb-6 break-words">
        ...
        <div className="mb-6 sm:mb-8">
          <h3 className="text-sm sm:text-base font-bold mb-3 sm:mb-4 uppercase tracking-wide border-b-2 border-purple-400 pb-2">
            Contact
          </h3>
        ...
        {skills.length > 0 && (
          <div className="mb-6 sm:mb-8">
            <h3 className="text-sm sm:text-base font-bold mb-3 sm:mb-4 uppercase tracking-wide border-b-2 border-accent-400 pb-2">
              Skills
            </h3>
              ...
              {skills.map((skill) => (
                <div key={skill.id} className="space-y-1 sm:space-y-2">
                  <div className="flex justify-between items-center gap-2">
                    <span className="text-white font-medium text-xs sm:text-sm truncate">{skill.name}</span>
                    <span className="text-accent-200 text-xs flex-shrink-0">{skill.level}</span>
                  </div>
            ...
            <div className="space-y-3 sm:space-y-4">
              {education.map((edu) => (
                <div key={edu.id} className="space-y-1">
                  <h4 className="text-white font-bold text-xs sm:text-sm break-words">{edu.degree}</h4>
                  <p className="text-accent-200 text-xs sm:text-sm break-words">{edu.school}</p>
                  {edu.field && (
                    <p className="text-accent-300 text-xs break-words">{edu.field}</p>
                  )}
      ...
      <div className="w-full xl:w-3/5 p-4 sm:p-6 xl:p-8">
        ...
        {personalInfo.summary && (
          <div className="mb-6 sm:mb-8">
            <h3 className="text-lg sm:text-xl font-bold text-accent-900 mb-3 sm:mb-4 uppercase tracking-wide border-l-4 border-accent-400 pl-3 sm:pl-4">
              About Me
            </h3>
        ...
        {experiences.length > 0 && (
          <div className="mb-6 sm:mb-8">
            <h3 className="text-lg sm:text-xl font-bold text-accent-900 mb-4 sm:mb-6 uppercase tracking-wide border-l-4 border-accent-400 pl-3 sm:pl-4">
              Experience
            </h3>
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-col gap-1 sm:gap-2">
                      <div className="flex-1">
                        <h4 className="font-bold text-accent-900 text-sm sm:text-base break-words">
                          {exp.position}
                        </h4>
                        <p className="text-accent-600 font-bold text-xs sm:text-sm mt-1 break-words">
                          {exp.company}
                        </p>
                      </div>
